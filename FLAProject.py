...
nfa = {}                                 
n = int(input("Number of states : "))          	        #Enter the number of states
t = int(input("Number of transitions : "))  	        #Enter the total number of transitions/paths eg: a,b so input 2 for a,b,c input 3
for i in range(n):  
    state = input("State name : ")	                #Enter name of state eg: A, B, C, q1, q2 ..etc
    nfa[state] = {}    		                        #Creating a nested dictionary
    for j in range(t):
        path = input("path : ")               		#Enter path eg : a or b in {a,b} 0 or 1 in {0,1}
        print("End state from state {} travelling through path {} : ".format(state,path))
...
print("\nFinal states are : ",dfa_final_states)       #Printing Final states of DFA